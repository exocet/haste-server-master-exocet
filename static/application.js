...
function formatDatePretty(epochTime){
    var currentTime = new Date().getTime();
    var diff = (currentTime - epochTime) / 1000;
    if(diff < 60){
      return "-1 minute";
    }else if(diff < 600){
        return "-10 minutes";
...
haste.prototype.loadLastest = function(){
  var _this = this;
  $.ajax('/list', {
    type: 'get',
    dataType: 'json',
    success: function(res) {
        ...
        $.each(res.list, function(key,val){
            var literalDate = new Date(val.timestamp);            
            html = html + "<li style=\"color:#009999;\" title=\"" + literalDate + "\">" + formatDatePretty(val.timestamp) + "</li>" 
                        + "<li><span class=\"content\"><a href=\""+ val.id + "\" title=\"" + literalDate + "</br></br>" + val.data + "(...)\">"+ val.data + "(...)</a></span></li>";              
        });
        $('#box4').append(html);
    },
    error: function(err) {      
